...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Redis (beta)](https://www.cdata.com/download/download.aspx?sku=EIZK-V&type=beta). 
## Purpose
...
CData JDBC Driver connects to Redis by exposing them as relational SQL models.

This server wraps that driver and makes Redis data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/redis/download/jdbc](https://www.cdata.com/drivers/redis/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Redis\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Redis/`
    * Run the command `java -jar cdata.jdbc.redis.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.redis.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `redis.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.redis.RedisDriver)
    * **JdbcUrl** - the JDBC connection string to use with the CData JDBC Driver to connect to your data (copied from above)
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=redis
      ServerName=CDataRedis
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.redis.jar
      DriverClass=cdata.jdbc.redis.RedisDriver
      JdbcUrl=jdbc:redis:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
